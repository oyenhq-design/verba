...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (parsedJson.sections) {
      parsedJson.sections.forEach((section: { blocks?: { text?: string, type?: string }[] }) => {
        section.blocks?.forEach((block: { text?: string, type?: string }) => {
          if (block.text && (block.type === 'paragraph' || block.type === 'heading')) {
